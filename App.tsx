    ...
    <div className="relative w-full h-screen bg-black overflow-hidden font-sans select-none text-white touch-none">
      ...
      <div className="relative z-50 w-full h-full flex flex-col pointer-events-none">
        ...
        <div className="p-4 pt-safe flex justify-between items-start pointer-events-auto">
          <div className="flex flex-col gap-1">
             ...
             <div className="font-black text-xl text-white flex items-center gap-2 drop-shadow-md">
               <Flame className="text-red-500 fill-red-500" /> 消防卫士
             </div>
          </div>
          ...
          <div className="flex gap-3">
            <button 
              ...
              className="p-3 bg-slate-800/80 backdrop-blur rounded-full text-slate-400 hover:text-white border border-slate-600 active:scale-95"
            >
              <RotateCcw size={20} />
            </button>
            <button 
              onClick={() => setIsChatOpen(true)}
              className="p-3 bg-red-600 rounded-full text-white shadow-lg animate-pulse-fast active:scale-95"
            >
              <Info size={20} />
            </button>
          </div>
        ...
        {step === SimulationStep.INTRO && (
          <div className="absolute inset-0 flex flex-col items-center justify-center p-8 text-center bg-black/80 backdrop-blur-md pointer-events-auto">
            <div className="w-32 h-32 bg-gradient-to-tr from-red-600 to-orange-500 rounded-full flex items-center justify-center mb-6 shadow-2xl animate-in zoom-in duration-500">
              <Flame size={64} className="text-white fill-white/20" />
            </div>
            <h1 className="text-4xl font-black mb-4 text-white">灭火器模拟训练</h1>
            <p className="text-slate-300 mb-10 max-w-sm leading-relaxed">
              通过高仿真 3D 互动，学习标准的 "提拔握压" (PASS) 灭火流程。
            </p>
